...
on __embedded_lscr103_block10
  ...
  if not (objRace <= maxPsi) then -- jump 30 -- 0x0028
  ...
  objRace -- 0x00AB
  if not (tqArray < length) then -- jump 286 -- 0x00BB
  if not (objRace > revLimiter) then -- jump 6 -- 0x00D0
  -- jump 262 -- 0x00D4
  ...
  if not (_global < (objRace * turboFlow)) then -- jump 18 -- 0x01B4
  push(VOID) -- 0x01C3
  -- jump 15 -- 0x01C4
