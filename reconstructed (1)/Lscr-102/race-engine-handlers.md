...
## Proven dyno and engine field names

The current bin-backed `Lscr-102` recovery preserves these RaceEngine names verbatim:

- `boostSetting` and `boostType` in `rightful.ls:181-303`
- `stockRedLine` and `redLine` in `rightful.ls:181-303`
- `maxPsi` and `chipSetting` in `rightful.ls:1736-1861`
- `tqFlyFactor`, `partBreakPoint`, and `vToMph` in `rightful.ls:51-99` and `rightful.ls:1784-1786`
- `c1320` and `c760` in `rightful.ls:95-96` and `rightful.ls:1816-1817`

The proven dyno callback sink in `reconstructed (1)/Lscr-93/rightful.ls:591-601` parses `d` and forwards XML attrs `e`, `u`, `ad`, `sl`, and `n` into `garageDynoBuyCB(...)`.

The proven engine callback sink in the FFDec export `tmp_ffdec_3_export/scripts/frame_12/DoAction.as:8481-8507` parses `d` and reads only:

- `es` -> `classes.GlobalData.engineSound`
- `sl` -> `classes.GlobalData.shiftLightGaugeRPM`
- `sg` -> `classes.GlobalData.hasShiftLightGauge`
- `rc` -> `classes.GlobalData.raceControlsID`
- `tmp` -> `classes.GlobalData.temp`

That is the recovered reason the live backend should keep `getonecarengine` and `buydyno` aligned on a shared `d`/`t` engine payload while separately preserving the two client-facing attr subsets:

- dyno callback subset: `e/u/ad/sl/n`
- engine callback subset: `es/sl/sg/rc/tmp`
