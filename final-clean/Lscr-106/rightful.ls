...
on downloadAvatar
  ...
  split = param_split.split(",") -- 0x0021
  "," = Array() -- 0x002C
  Array = VOID -- 0x0031
  if not (Array < split.length) then -- jump 211 -- 0x003E
  op_6d(aid) -- 0x004F
  if not (param_Array = TRUE) then -- jump 20 -- 0x0054
  ...
  executeCall(getavatarage, ("tids=" & ",".join(","))) -- 0x0142
  -- jump 41 -- 0x0143
  if not (param_"," = teamavatars) then -- jump 31 -- 0x014C
  executeCall(getteamavatarage, ("tids=" & ",".join(","))) -- 0x016B
end
...
on compareAvatarAge
  ...
  clearCache() -- 0x001B
  clearCache = ((((_global.avatarMainURL & param__global) & "\\") & avatarIDtoPath(param_clearCache)) & ".jpg") -- 0x0039
  ...
  _global.fileNetID.push(_global) -- 0x007F
end
...
on loadLocalAvatar
  ...
  baLingoFileAge = baLingoFileDateEx(baLingoTempFileName, accessed, "yyyy|m|d|", "h|n|s").split("|") -- 0x004D
  baLingoFileDateEx = VOID -- 0x0052
  if not (baLingoFileDateEx < param_baLingoTempFileName.length) then -- jump 309 -- 0x005F
  -- 0x3F property-chain marker -- 0x006A
  if not (implicit_object_0x006B[param_baLingoTempFileName[baLingoFileDateEx]] > -1) then -- jump 228 -- 0x006F
  -- 0x3F property-chain marker -- 0x00A2
  if not (implicit_lhs_0x00AC > (((avage - baLingoFileAge((((((cacheFolder & "\\") & param_avage) & "\\") & param_baLingoTempFileName[baLingoFileDateEx][VOID]) & ".jpg")))[param_baLingoTempFileName[baLingoFileDateEx]] - fileAgeCheck) - 360)) then -- jump 26 -- 0x00AC
  ...
  -- 0x3F property-chain marker -- 0x0110
  implicit_callee_0x014D(baLingoSetFileDate, (((((cacheFolder & "\\") & param_avage) & "\\") & param_baLingoTempFileName[baLingoFileDateEx][VOID]) & ".jpg"), parseInt(baLingoFileAge[VOID])(parseInt[baLingoFileAge]), parseInt(baLingoFileAge[baLingoFileAge]), parseInt(baLingoFileAge[baLingoFileDateEx]), parseInt(baLingoFileAge[accessed]), parseInt(baLingoFileAge["yyyy|m|d|"])) -- 0x0150
  -- jump 61 -- 0x0151
...
on avatarIDtoPath
  ...
  sprite(flashSP).getAvatarCB(param_sprite, param_flashSP, param_getAvatarCB) -- 0x002E
end
...
on downloadUgg
  ...
  (Math.floor & param_Math(100).toString()) -- 0x008D
end
...
on loadLocalUgg
  ...
  _global = ((_global.uggURL & "cache/car/userDecals/") & param__global) -- 0x0022
  ...
  ugg -- 0x004B
  _global.fileNetID.push(uggURL) -- 0x005C
end
...
on fileBrowse
  ...
  sprite(flashSP).downloadUggCB(param_sprite, param_flashSP) -- 0x002B
end
...
on uggUploadSuccess
  ...
  _global = ((((_global.uggURL & "/Upload.aspx?NeatUpload_PostBackID=") & _global.aid) & "&accountID=") & _global.aid) -- 0x0030
  uggURL = Object() -- 0x003B
  gSimInet.FieldClear() -- 0x0049
  -- branch 20 -- 0x0050
  gSimInet.AddFieldFile(hoodImage, param__global) -- 0x0063
  -- branch 20 -- 0x006A
  gSimInet.AddFieldFile(sideImage, param_uggURL) -- 0x007D
  -- branch 20 -- 0x0084
  gSimInet.AddFieldFile(frontImage, param_"/Upload.aspx?NeatUpload_PostBackID=") -- 0x0097
  -- branch 20 -- 0x009E
  gSimInet.AddFieldFile(backImage, param_aid) -- 0x00B1
  gSimInet.SendField1(_global, uggUploadSuccess, uggUploadFail) -- 0x00C5
end
...
on uggUploadFail
  ...
  sprite(flashSP).uggUploadCB(TRUE, param_sprite) -- 0x0029
end
...
on downloadInstallerUpdateFlash
  ...
  baLingoSysFolder = (baLingoSysFolder(temp) & _global.installerFile) -- 0x0025
  -- branch 118 -- 0x0033
  temp = new xtra("caluMD5") -- 0x0040
  baLingoFileExists(baLingoSysFolder) -- 0x0043
  if not (temp.getFileMD5(baLingoSysFolder) = _global.installerMD5) then -- jump 17 -- 0x0057
  runInstaller(baLingoSysFolder) -- 0x0065
  -- jump 64 -- 0x0066
  -- 0x03 packed assignment prefix raw=03 -- 0x0090
  _global = (baLingoSysFolder ^ downloadNetThing(((_global.installerURL & "/") & _global.installerFile), baLingoSysFolder)) -- 0x0091
  installer -- 0x0094
  _global.fileNetID.push(_global) -- 0x00A5
  -- jump 64 -- 0x00A6
  -- 0x03 packed assignment prefix raw=03 -- 0x00D0
  _global = (baLingoSysFolder ^ downloadNetThing(((_global.installerURL & "/") & _global.installerFile), baLingoSysFolder)) -- 0x00D1
  installer -- 0x00D4
  _global.fileNetID.push(_global) -- 0x00E5
end
...
on runInstaller
  ...
  sprite(flashSP).loadUpdateCB(_global.installerFile, param_sprite.bytesSoFar, param_sprite.bytesTotal, param_sprite.bytesSoFar, param_sprite.bytesTotal) -- 0x0043
end
...
on fetchCacheXML
  ...
  _player.quit() -- 0x003E
  -- jump 27 -- 0x003F
  sprite(flashSP).showConnectionErrorCB("Download Failed", "I'm sorry, but the automatic updater is not working properly. Please visit http://www.nittolegends.com and download the latest version.") -- 0x0059
end
...
on fetchCacheXMLDone
  ...
  (getcache ^ getNetText((_global.cacheURL & "content.htm"))) -- 0x006C
  clearCache = Array -- 0x006D
  (op_6c(aryCacheFileSizes) ^ Array) -- 0x0070
  _global.gNetID.push(clearCache) -- 0x0081
end
...
on downloadCache
  ...
  count = t -- 0x0017
  if not (count <= param_"!ATTRIBUTES".count) then -- jump 429 -- 0x0024
  -- 0x37 property-chain marker -- 0x002E
  if not (count.!ATTRIBUTES.t = d) then -- jump 95 -- 0x0038
  -- 0x37 property-chain marker -- 0x0049
  -- branch 28 -- 0x0055
  -- 0x37 property-chain marker -- 0x0065
  param_count((param_"!ATTRIBUTES" & count.!ATTRIBUTES.n)) -- 0x0070
  -- 0x37 property-chain marker -- 0x007E
  param_count(((param_"!ATTRIBUTES" & count.!ATTRIBUTES.n) & "\\"), param_"!ATTRIBUTES"[count]) -- 0x0094
  -- jump 310 -- 0x0095
  -- 0x37 property-chain marker -- 0x009E
  if not (count.!ATTRIBUTES.t = f) then -- jump 290 -- 0x00A8
  "!ATTRIBUTES" = new xtra("caluMD5") -- 0x00B6
  param_"!ATTRIBUTES" -- 0x00B9
  -- 0x37 property-chain marker -- 0x00C7
  -- branch 97 -- 0x00D3
  -- 0x37 property-chain marker -- 0x00E6
  param_count((param_"!ATTRIBUTES" & count.!ATTRIBUTES.n)) -- 0x00F1
  -- 0x37 property-chain marker -- 0x0103
  parseInt(param_"!ATTRIBUTES"(count.!ATTRIBUTES.s)) -- 0x0110
  -- 0x37 property-chain marker -- 0x0122
  op_6d(totalCacheSize) -- 0x0130
  -- jump 138 -- 0x0131
  -- 0x37 property-chain marker -- 0x0144
  -- 0x37 property-chain marker -- 0x0155
  if not (param_"!ATTRIBUTES" <> count.!ATTRIBUTES.md) then -- jump 94 -- 0x015C
  -- 0x37 property-chain marker -- 0x0170
  param_count((param_"!ATTRIBUTES" & count.!ATTRIBUTES.n)) -- 0x017B
  -- 0x37 property-chain marker -- 0x018D
  parseInt(param_"!ATTRIBUTES"(count.!ATTRIBUTES.s)) -- 0x019A
  -- 0x37 property-chain marker -- 0x01AC
  op_6d(totalCacheSize) -- 0x01BA
  "!ATTRIBUTES" = VOID -- 0x01BC
  (parseInt & param_"!ATTRIBUTES"(count.!ATTRIBUTES.s)) -- 0x01BF
  _system.gc() -- 0x01CA
  -- repeat step 0 -- 0x01CB
  op_6c(totalCacheSize).totalCacheSize -- 0x01CE
  -- jump -436 -- 0x01CF
end
...
on downloadCacheInProgress
  ...
  cache -- 0x0072
  _global.fileNetID.push(_global) -- 0x0083
end
...
on downloadCacheDone
  ...
  downloadCacheUpdateFlash(param_downloadCacheUpdateFlash.bytesSoFar) -- 0x0021
end
...
on downloadCacheUpdateFlash
  ...
  (op_6c(totalDownloadedCacheSize).totalDownloadedCacheSize & aryCacheFileSizes[aryCacheIndex]) -- 0x0032
  if not (aryCacheIndex < aryCacheFiles.length) then -- jump 21 -- 0x003C
  downloadCache(aryCacheFiles[aryCacheIndex]) -- 0x004E
  -- jump 21 -- 0x004F
  sprite(flashSP).loadFinishCB() -- 0x0063
end
...
on loadDyno
  ...
  sprite(flashSP).loadUpdateCB(aryCacheFiles[aryCacheIndex], param_sprite, aryCacheFileSizes[aryCacheIndex], (totalDownloadedCacheSize & param_sprite), totalCacheSize) -- 0x0040
end
...
on saveDyno
  ...
  fileio = xtra.displayOpen() -- 0x003A
  -- branch 116 -- 0x0041
  xtra.openFile(fileio, TRUE) -- 0x0052
  if not (xtra.status() = VOID) then -- jump 86 -- 0x005E
  setFilterMask = xtra.readFile() -- 0x006C
  fileio -- 0x006F
  setFilterMask = setFilterMask.split("|") -- 0x007D
  -- 0x3F property-chain marker -- 0x00A1
  implicit_callee_0x00B1(sprite(flashSP).garageDynoLoadCB, getTxtFileName(fileio), setFilterMask[VOID][setFilterMask], setFilterMask[setFilterMask], setFilterMask["Text Files,*.txt"]) -- 0x00B4
  xtra.closeFile() -- 0x00BF
end
...
on getTxtFileName
  ...
  xtra.writeString(((((((param_xtra & "|") & param_fileio) & "|") & param_setFilterMask) & "|") & param_"Text Files,*.txt")) -- 0x008E
  xtra.closeFile() -- 0x0099
  sprite(flashSP).garageDynoSaveCB(getTxtFileName(fileio)) -- 0x00B5
end
...
on saveFile
  ...
  length = param_length.length -- 0x001A
  if not (length >= VOID) then -- jump 60 -- 0x0022
  if not (param_length.charAt(length) = "\\") then -- jump 33 -- 0x0036
  param_length.substr((length & TRUE), ((param_length.length - length) - 5)) -- 0x0057
...
on openfile
  ...
  xtra = new xtra("fileio") -- 0x001E
  xtra.createFile(param_xtra) -- 0x002F
  xtra.openFile(param_xtra, createFile) -- 0x0040
  xtra.writeString(unescape(param_fileio)) -- 0x0055
  if not (xtra.status() = VOID) then -- jump 28 -- 0x0061
  sprite(flashSP).saveFileCB(TRUE, param_xtra) -- 0x007A
  -- jump 25 -- 0x007B
  sprite(flashSP).saveFileCB(VOID, param_xtra) -- 0x0093
  xtra.closeFile() -- 0x009E
end
...
on fileClearAllDecals
  ...
  xtra = new xtra("fileio") -- 0x001E
  xtra.openFile(param_xtra, TRUE) -- 0x0030
  if not (xtra.status() = VOID) then -- jump 38 -- 0x003C
  sprite(flashSP).openFileCB(TRUE, param_xtra, xtra.readFile()) -- 0x005F
  -- jump 25 -- 0x0060
  sprite(flashSP).openFileCB(VOID, param_xtra) -- 0x0078
  xtra.closeFile() -- 0x0083
end
...
on fileClearADecal
  ...
  VOID -- 0x0029
  cacheFolder = TRUE -- 0x002B
  if not (cacheFolder <= baLingoFileList.count) then -- jump 33 -- 0x0038
...
on __embedded_lscr106_block30
  ...
  -- jump -127 -- 0x00E1
  xtra.closeFile() -- 0x00EE
  socketURL = openFile[VOID] -- 0x00F7
  _global -- 0x00FA
  -- 0x3F property-chain marker -- 0x0101
  socketPort = _global[openFile] -- 0x0103
  mainURL = openFile[openFile] -- 0x0111
  _global -- 0x0114
  gameURL = (_global.mainURL & "gameCode1_00.aspx") -- 0x0122
  _global -- 0x0125
...
-- evidence markdown: output/evidence/Lscr-106.aggressive-evidence.md
-- evidence json: output/evidence/Lscr-106.aggressive-evidence.json
-- blocks=31 actions=0 branches=27 methodCalls=50
-- literalNames=159 callbackLikeNames=12 constants=7
-- todos=0 implicitSites=7
