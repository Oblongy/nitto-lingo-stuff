...
on __embedded_lscr93_block01
  ...
  -- pass26 action-condition chain-member: checkedForOneClient.getAProp("s") = -2 -- promoted from block1-table slot 59 (0x37E4) via opcode 0x3B at 0x05FB
  -- pass22 structured-control: else
  if not (checkedForOneClient.getAProp("s") = -2) then -- jump 60 marker 0x44 -- 0x05FC
  -- pass26 action-condition chain-member: checkedForOneClient.getAProp("s") = -11 -- promoted from block1-table slot 60 (0x37F0) via opcode 0x3C at 0x060F
  ...
  -- pass26 action-condition: if checkedForOneClient.getAProp("s") = -99 then -- promoted from block1-table slot 62 (0x3808) via opcode 0x3E at 0x0637; false branch jumps to 0x066A
  ...
  -- pass26 action-condition: if checkedForOneClient.getAProp("s") = -14 then -- promoted from block1-table slot 70 (0x38C8) via opcode 0x46 at 0x06F9; false branch jumps to 0x0733
  ...
  -- pass26 action-condition: if checkedForOneClient.getAProp("s") = -15 then -- promoted from block1-table slot 74 (0x392C) via opcode 0x4A at 0x0743; false branch jumps to 0x0769
  ...
  -- pass22 structured-control: else
  sprite(flashSP).raceGetTwoRacersCarsCB(checkedForOneClient.getAProp("s"), "") -- 0x1EFB
  -- jump 3349 -- 0x1EFC
  -- pass22 structured-control: end if
  ...
  -- pass22 structured-control: else
  sprite(flashSP).getRacersCarsCB(checkedForOneClient.getAProp("s"), "") -- 0x1F61
  -- jump 3247 -- 0x1F62
  -- pass22 structured-control: end if
  ...
  sprite(flashSP).htJoinTournamentCB(checkedForOneClient.getAProp("s"), checkedForOneClient.getAProp("b"), "") -- 0x228F
  raceType = HTQ -- 0x2299
  ...
  -- pass22 structured-control: else
  sprite(flashSP).htJoinTournamentCB(checkedForOneClient.getAProp("s"), "", checkedForOneClient.getAProp("d")) -- 0x22F6
  -- jump 2330 -- 0x22F7
  -- pass22 structured-control: end if
  ...
  -- jump 71 -- 0x2C35
  sprite(flashSP).showConnectionErrorCB("Connection Error", (((("Connection could not be established. Action:" & _global.gNetID[_global][VOID]) & " (Error: 003-") & netError[_global.gNetID[_global]]) & ")")) -- 0x2C78
  -- jump 3 -- 0x2C79
