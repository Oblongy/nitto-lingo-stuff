...
-- Source: Lscr-108.json / Lscr-108.decompiled.lingo
-- Constraint: recovered bytecode evidence only; no source invented.
-- Note: identical handler set to Lscr-101 (scriptNumber=8, 15 handlers).
-- Lscr-108 totalLength=1928 vs Lscr-101 totalLength=1916 (12-byte padding diff).
...
on lingoXtraInit
  ...
  set the timeoutLength to 216000
  set the timeoutScript to "setInactive"
...
on getPCInfo
  ...
  if offset(".", bdi) > 0 then
    bdi = bdi.char[1..offset(".", bdi) - 1]
  end if
  qs = "mid=" & bdi & baCpuInfo("vendor") & baCpuInfo("type") & baCpuInfo("model")
  qs = qs & "&nid=" & To3GetMacAddress().char[1..17]
...
on isCheatEngineRunning
  ceHandle = baWindowList(EMPTY, "ch" & "eat" & " en" & "gine", 0)
