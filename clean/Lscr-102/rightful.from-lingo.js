...
class Lscr102RaceEngine {
  constructor(host = {}) {
    this.host = host;
    this.handlers = this.RaceEngine_dispatchTable();
    this.RaceEngine_constantsTable();
    this.RaceEngine_initState();
  }

  RaceEngine() {
    return this.RaceEngine_dispatchTable();
  }

  RaceEngine_dispatchTable() {
    return {
      1: this.RaceEngine_constantsTable.bind(this),
      2: this.RaceEngine_initState.bind(this),
      3: this.RaceEngine_parseConfig.bind(this),
      4: this.RaceEngine_initBoost.bind(this),
      5: this.RaceEngine_startSendTimer.bind(this),
      6: this.RaceEngine_stopRunTimer.bind(this),
      7: this.RaceEngine_stageDistance.bind(this),
      8: this.RaceEngine_runFrame.bind(this),
      9: this.RaceEngine_calculateExtraHpi.bind(this),
      10: this.RaceEngine_lookupTorque.bind(this),
      11: this.RaceEngine_initBrakeTune.bind(this),
      12: this.RaceEngine_gearDown.bind(this),
      13: this.RaceEngine_gearUp.bind(this),
      14: this.RaceEngine_setBrake.bind(this),
      15: this.RaceEngine_setClutchFeather.bind(this),
      16: this.RaceEngine_createMd5Hash.bind(this),
      17: this.RaceEngine_startLightTimer.bind(this),
      18: this.RaceEngine_runLight.bind(this),
      19: this.RaceEngine_buildMd5Source.bind(this),
      20: this.RaceEngine_randomCurve.bind(this),
    };
  }
  ...
  RaceEngine_parseConfig(carXML) {
    ...
    this.RaceEngine_setClutchFeather(0);
  }
  ...
  RaceEngine_runFrame(frameInput = 0) {
    ...
    const tq = this.RaceEngine_lookupTorque(this.rpm);
  ...
  RaceEngine_calculateExtraHpi(rpm) {
    rpm = rpm / this.pb;
    if (rpm > 1) rpm = 1;
  ...
  RaceEngine_initBrakeTune(init, btd) {
    init?.(init);
    this.btd = btd;
  }
  ...
  RaceEngine_setBrake(brakeState) {
    this.brake = brakeState ? 1 : 0;
  }

  RaceEngine_setClutchFeather(clutchValue) {
    if (clutchValue < 0.05) {
  ...
  RaceEngine_startLightTimer(greenDelay) {
  ...
  RaceEngine_runLight(period, name) {
    period = 500;
...
class Lscr102MechanicalTrace {
  ...
  __stack_Lscr102_block08() {
    ...
    this.__branch(block, "if not (implicit_lhs_0x11C5 <> (rt mod c1)) then -- jump 23", "0x11C5");
    ...
    this.__branch(block, "if not (implicit_lhs_0x1254 <> (rt mod c1)) then -- jump 35", "0x1254");
  ...
  __stack_Lscr102_block19() {
    ...
    this.__assign(block, "stockRedLine", "TRUE", "0x006C");
    ...
    this.__unresolved(block, "?", "0x05BA");
    this.__unresolved(block, "packet-only engineDamage/c0 op_14 family: raw order engineDamage -> c0 -> branch-consumer target/imm=14; strict surface order ? -> op_14 marker -> null.c0 -> null.engineDamage -> // goto @1473", "0x05BB");
    this.__assign(block, "id_0", "(? + nogood)", "0x05CE");
    this.__unresolved(block, "?", "0x05D1");
    this.__expr(block, "param_id_0", "0x05D2");
    this.__control(block, "jump 106", "0x05D5");
    this.__expr(block, "goodCounterRT", "0x05DC");
    this.__branch(block, "if false then -- else goto @1494", "0x05DD");
    this.__expr(block, "goodCounterET", "0x05E5");
    this.__branch(block, "if false then -- else goto @1511", "0x05E6");
    this.__assign(block, "id_0", "(? + nogood)", "0x05F1");
    this.__unresolved(block, "?", "0x05F4");
    this.__control(block, "goto @1507", "0x05F5");
    this.__unresolved(block, "packet A compare core remains packet-only: duplicated badCounterRT load -> goodCounterRT load -> duplicated badCounterRT load -> terminal combine", "0x05F8");
    this.__unresolved(block, "packet A rhs ladder remains packet-local: op_83 arg=7745 -> lagPercent -> branch-consumer target/imm=17 -> goto @1548", "0x0605");
    this.__assign(block, "id_0", "(? + nogood)", "0x0616");
    this.__unresolved(block, "?", "0x0619");
    this.__control(block, "goto @1544", "0x061A");
    this.__unresolved(block, "packet B compare core remains packet-only: duplicated badCounterET load -> goodCounterET load -> duplicated badCounterET load -> terminal combine", "0x061D");
    this.__unresolved(block, "packet B rhs ladder remains packet-local: op_83 arg=7745 -> lagPercent -> branch-consumer target/imm=14 -> goto @1582", "0x062A");
    this.__assign(block, "id_0", "(? + nogood)", "0x063B");
    this.__unresolved(block, "?", "0x063E");
    this.__expr(block, ", = caluMD5", "0x0649");
    this.__assign(block, "id_0", ",.getStringMD5()(id_0)", "0x0650");
    this.__expr(block, "id_0", "0x0663");
    this.__expr(block, "end", null);
    return this.trace;
  }
...
module.exports = {
  Lscr102OriginalStyle: Lscr102RaceEngine,
  Lscr102RaceEngine,
